...
function fetchRecentCareerStories() {
    ...
    fetch('/recent-career-stories/')
            ...
            .then(statusData => {
                ...
                data.careers.forEach(story => {
                    story.activity_status =
                        statusData?.statuses?.[story.id] || 'inactive';
        ...
        .catch(error => {
            console.error('最近のキャリア取得エラー:', error);
        });
}
...
function createStoryCard(story) {
    ...
    const age =
        story.birthYear
            ? calculateAge(story.birthYear)
            : '不明';

    const activityStatus =
        story.activity_status || 'inactive';

    const statusLabel = RECENT_STATUS_LABELS[activityStatus] || RECENT_STATUS_LABELS.inactive;

    const tags =
        typeof generateCareerTags === 'function'
            ? generateCareerTags(story)
            : [];

    const careerTypeText =
        simplifyCareerType(story.career_type);
    ...
    const cardHeader = `
        <div class="card-header">
            ...
            <p>
                職業:
                ${escapeHTML(story.profession || '不明')}
            </p>

            <p>
                年収:
                ${escapeHTML(String(latestIncome))}
            </p>

            ${careerTypeText ? `
                <p>
                    志向:
                    ${escapeHTML(careerTypeText)}
                </p>
            ` : ''}
    ...
    const viewCountSection = `
        <div class="card-footer">
            ...
            <span class="view-count">
                ${escapeHTML(String(story.view_count || 0))} 回
            </span>
        </div>
    `;
    ...
    card.addEventListener('click', function () {
        ...
        window.location.href =
            `Career_detail.html?id=${story.id}`;
    });
...
function setupIndicators(sectionId, containerId) {
    ...
    const indicatorsContainer =
        document.getElementById(`${sectionId}-indicators`);
    ...
    cards.forEach((card, index) => {
        ...
        indicator.addEventListener('click', () => {
            ...
            const cardWidthWithMargin =
                cards[0].offsetWidth +
                parseInt(
                    getComputedStyle(cards[0]).marginRight || 0
                );
    ...
    cardsContainer.addEventListener('scroll', () => {
        ...
        const index =
            Math.round(
                cardsContainer.scrollLeft / cardWidthWithMargin
            );
...
function updateIndicators(sectionId, activeIndex) {
    ...
    const indicators =
        document.querySelectorAll(
            `#${sectionId}-indicators .indicator`
        );
...
function simplifyCareerType(type) {
    ...
    if (type.includes('給与') || type.includes('収入UP')) {
        return '年収UP志向';
    }

    if (type.includes('安定収入')) {
        return '安定志向';
    }

    if (type.includes('ワークライフバランス')) {
        return '働き方重視';
    }

    if (type.includes('フレックス')) {
        return '柔軟な働き方';
    }

    if (type.includes('人間関係')) {
        return '人間関係重視';
    }
    ...
    if (type.includes('専門技術')) {
        return '専門性UP';
    }

    if (type.includes('マネジメント')) {
        return 'マネジメント志向';
    }
